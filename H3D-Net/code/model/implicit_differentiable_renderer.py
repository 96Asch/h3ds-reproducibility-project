...
from model.igr import GeometryNetwork
...
class IDRGeometryNetwork(nn.Module):
    def __init__(
            self,
            d_in,
            d_out,
...
class IDRNetwork(nn.Module):
    def __init__(self, conf):
        ...
        # self.geometry_network = ImplicitNet(**conf.get_config('geometry_network'))
        self.geometry_network = IDRGeometryNetwork(**conf.get_config('geometry_network'))
        self.appearance_network = AppearanceNetwork(self.feature_vector_size, **conf.get_config('appearance_network'))
        self.rendering_network = RenderingNetwork(self.feature_vector_size, **conf.get_config('rendering_network'))
        self.ray_tracer = RayTracing(**conf.get_config('ray_tracer'))
        self.sample_network = SampleNetwork()
        self.object_bounding_sphere = conf.get_float('ray_tracer.object_bounding_sphere')

    def load_latent(self, latents):
        self.latent_vecs = latents
    
    def set_indices(self, indices):
        self.indices = indices

    def add_latent(self, points):
        num_of_points, dim = points.shape
        points = points.reshape(num_of_points, dim)
        latent_inputs = torch.zeros(0).cuda()
        print(self.latent_vecs)
        # print(points.shape)
        for ind in self.indices.numpy():
            latent_ind = self.latent_vecs[ind]
            latent_repeat = latent_ind.expand(num_of_points, -1)
            latent_inputs = torch.cat([latent_inputs, latent_repeat], 0)
        # print(f"latent:{latent_inputs.shape}")
    ...
    def forward(self, input):
        ...
        self.appearance_network.eval()
       
        with torch.no_grad():
        #     points, network_object_mask, dists = self.ray_tracer(sdf=lambda x: self.geometry_network(self.add_latent(x)),
        #                                                          cam_loc=cam_loc,
        #                                                          object_mask=object_mask,
        #                                                          ray_directions=ray_dirs)
             points, network_object_mask, dists = self.ray_tracer(sdf=lambda x: self.geometry_network(x),
                                                                 cam_loc=cam_loc,
                                                                 object_mask=object_mask,
                                                                 ray_directions=ray_dirs)
        self.geometry_network.train()
        self.appearance_network.train()

        points = (cam_loc.unsqueeze(1) + dists.reshape(batch_size, num_pixels, 1) * ray_dirs).reshape(-1, 3)

        sdf_output = self.geometry_network(points)
        # sdf_output = self.geometry_network(self.add_latent(points))
        ...
        if self.training:
            ...
            sdf_output = self.geometry_network(points)
            surface_sdf_values = sdf_output[:N].detach()
