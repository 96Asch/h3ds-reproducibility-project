...
class IDRTrainRunner():
    def __init__(self,**kwargs):
        ...
        print("Loading old IGR Checks")
        old_checkpnts_dir = os.path.join('../igrexps/', '60ep', 'checkpoints')
        # print(old_checkpnts_dir)
        data = torch.load(os.path.join(old_checkpnts_dir, self.latent_codes_subdir, "59" + '.pth'))
        self.model.load_latent(data["latent_codes"].cuda())
        # print(os.path.join(old_checkpnts_dir, 'ModelParameters', '59' + ".pth"))
        # saved_model_state = torch.load(os.path.join(old_checkpnts_dir, 'ModelParameters', '59' + ".pth"))
        # self.model.geometry_network.load_state_dict(saved_model_state["model_state_dict"])


        if is_continue:
            # Load IDR checkpoints         
