...
class SceneDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        img_indices = [int(path.split('/')[-1][4:8]) for path in image_paths]
        print(img_indices)
        ...
        scale_mats = [camera_dict['scale_mat_%d' % idx].astype(np.float32) for idx in img_indices]
        world_mats = [camera_dict['world_mat_%d' % idx].astype(np.float32) for idx in img_indices]
        self.intrinsics_all = []
        self.pose_all = []
