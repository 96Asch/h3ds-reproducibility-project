...
def plot(model, indices, model_outputs ,pose, rgb_gt, path, epoch, img_res, plot_nimgs, max_depth, resolution):
    ...
    surface_traces = get_surface_trace(path=path,
                                       epoch=epoch,
                                       sdf=lambda x: model.geometry_network(x),
                                       resolution=resolution
                                       )
    # surface_traces = get_surface_trace(path=path,
    #                                 epoch=epoch,
    #                                 sdf=lambda x: model.geometry_network(model.add_latent(x)),
    #                                 resolution=resolution
    #                                 )
    ...
    for i, p, m in zip(indices, points, network_object_mask):
        ...
        # val = model.geometry_network(model.add_latent(p))
        val2 = model.appearance_network(p)

        val = torch.cat((val, val2), 1)
...
def get_surface_trace(path, epoch, sdf, resolution=100, return_mesh=False):
    ...
    print(f"len: {len(z)} np.min:{np.min(z)}, np.max:{np.max(z)}")
